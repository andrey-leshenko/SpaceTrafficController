import {Point, Rectangle, dist} from './utils.js'
...
function getRandomChunk(min: number, max: number, quantization: number) {
...
function pointInBounds(point: Point, bounds: Rectangle) {
    return (
        point.x >= bounds.x &&
...
export function spawnLinePath(bounds: Rectangle) {
    let launch_pt = {
...
export function spawnCirclePath(bounds: Rectangle) {
    let launch_pt = {
        x: getRandomChunk(bounds.x, bounds.x + bounds.width, 8),
        y: getRandomChunk(bounds.y, bounds.y + bounds.height, 8)
    }
