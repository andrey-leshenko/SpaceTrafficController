import { PausableTimeout } from './utils.js';
...
export class Satellite {
    constructor(space, path, launch_pt) {
        this.pathFraction = 0;
        this.speed = 30;
        this.radius = 15;
        this.warningIconSize = 15;
    ...
    drawSelf() {
        ...
        let drawSelfAt = (x, y) => {
            this.space.ctx.drawImage(satelliteImage, x - this.radius, y - this.radius, this.radius * 2, this.radius * 2);
            if (this.collisionWarning) {
                this.space.ctx.drawImage(warningImage, x + this.radius - this.warningIconSize / 2, y - this.radius - this.warningIconSize / 2, this.warningIconSize, this.warningIconSize);
            }
            this.space.ctx.restore();
        };
        for (let p of this.getPosAtTime())
